// ─── Activity screen — full transaction history with filters ──

const { useState: useWAct, useMemo: useWActMemo } = React;

function WebActivity({ navigate }) {
  const [search, setSearch] = useWAct('');
  const [typeFilter, setTypeFilter] = useWAct('All');
  const [assetFilter, setAssetFilter] = useWAct('All');
  const [statusFilter, setStatusFilter] = useWAct('All');
  const [range, setRange] = useWAct('30D');

  // Filter logic
  const filtered = useWActMemo(() => {
    return WTXS.filter(tx => {
      ...
      if (search) {
        ...
        if (!tx.id.toLowerCase().includes(q) &&
            ...
            !String(tx.paid).toLowerCase().includes(q)) return false;
      }
      return true;
    });
      ...
      <WCard padding={0}>
        ...
        <div style={{ padding: '14px 22px', borderBottom: `1px solid ${WBRAND.line}` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
            {/* Search */}
            <div style={{
              height: 36, flex: '1 1 260px', minWidth: 240, maxWidth: 320,
              background: WBRAND.surface, borderRadius: 8, padding: '0 12px',
              display: 'flex', alignItems: 'center', gap: 8,
            }}>
              {WIcon.search()}
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by ID, asset, counterparty…" style={{
                flex: 1, border: 'none', outline: 'none', background: 'transparent',
                fontFamily: WFONT, fontSize: 13, color: WBRAND.ink,
              }}/>
            </div>
          ...
          {/* Active filter chips */}
          {(typeFilter !== 'All' || assetFilter !== 'All' || statusFilter !== 'All' || search) && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 12, flexWrap: 'wrap' }}>
              ...
              <button onClick={() => { setSearch(''); setTypeFilter('All'); setAssetFilter('All'); setStatusFilter('All'); }} style={{
        ...
        {/* Results count */}
        <div style={{
          padding: '12px 22px', borderBottom: `1px solid ${WBRAND.line}`,
        ...
        {filtered.length === 0 ? (
          <div style={{
            ...
            fontFamily: WFONT, fontSize: 14, color: WBRAND.muted,
          }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            ...
            {[1, 2, 3, 4].map(p => (
              <button key={p} style={{
                ...
                background: p === 1 ? WBRAND.ink : WBRAND.white,
                color: p === 1 ? '#fff' : WBRAND.ink,
                border: `1px solid ${p === 1 ? WBRAND.ink : WBRAND.line}`,
          ...
          <div style={{
            ...
            {options.map(o => (
              <button key={o} onClick={() => { onChange(o); setOpen(false); }} style={{
                ...
                background: o === value ? WBRAND.surface : 'transparent',
                cursor: 'pointer', textAlign: 'left',
                fontFamily: WFONT, fontSize: 12, fontWeight: o === value ? 700 : 500, color: WBRAND.ink,
...
function FilterChip({ label, onClear }) {
  return (
    <span style={{
      ...
      background: WBRAND.surface, padding: '4px 4px 4px 10px',
      borderRadius: 6, fontFamily: WFONT, fontSize: 11, fontWeight: 600, color: WBRAND.ink,
    }}>
      {label}
      <button onClick={onClear} style={{
        width: 18, height: 18, borderRadius: 4, border: 'none',
        background: 'transparent', cursor: 'pointer',
        display: 'grid', placeItems: 'center', color: WBRAND.muted,
      }}>
...
Object.assign(window, { WebActivity, FilterDropdown, FilterChip });
