// ─── Profile screen ───────────────────────────────────────────
// Left rail: profile summary + settings nav. Right: section content.

const { useState: useWProf } = React;

function WebProfile({ navigate }) {
  const [section, setSection] = useWProf('account');
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <WCard padding={20}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
              <div style={{
                ...
                fontFamily: WFONT, fontWeight: 800, fontSize: 26, letterSpacing: '-0.02em',
              }}>AY</div>
            ...
            {sections.map(s => {
              ...
              return (
                <button key={s.id} onClick={() => setSection(s.id)} style={{
                  ...
                  fontFamily: WFONT, fontSize: 13, fontWeight: on ? 700 : 500,
                  color: WBRAND.ink, letterSpacing: '-0.005em',
                }}>
          ...
          <WCard padding={16}>
            <button style={{
              ...
              fontFamily: WFONT, fontSize: 13, fontWeight: 600, color: WBRAND.red,
              textAlign: 'left',
...
function FormRow({ label, hint, children }) {
  return (
    <div style={{
      ...
      padding: '18px 22px', borderBottom: `1px solid ${WBRAND.line}`, alignItems: 'flex-start',
    }}>
      <div>
...
function SectionCard({ title, sub, children, footer }) {
  return (
    <WCard padding={0}>
      ...
      {footer && (
        <div style={{
          padding: '14px 22px', borderTop: `1px solid ${WBRAND.line}`,
          display: 'flex', justifyContent: 'flex-end', gap: 8,
          background: WBRAND.surface2, borderBottomLeftRadius: 16, borderBottomRightRadius: 16,
        }}>{footer}</div>
      )}
    </WCard>
  );
...
function ProfAccount() {
  return (
    <SectionCard
      ...
      sub="Information shown on your profile and used for verification."
      footer={<><WSecondary>Cancel</WSecondary><WPrimary>Save changes</WPrimary></>}
    >
  ...
  return (
    <>
      <SectionCard title="Verification status" sub="Your verification tier unlocks higher daily limits and physical delivery.">
        <div style={{ padding: '20px 22px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginBottom: 20 }}>
            <div style={{
              width: 64, height: 64, borderRadius: 12, background: WBRAND.redSoft,
              display: 'grid', placeItems: 'center', color: WBRAND.red,
              fontFamily: WFONT, fontSize: 22, fontWeight: 800, letterSpacing: '-0.02em',
            }}>T3</div>
            <div style={{ flex: 1 }}>
          ...
          <div style={{
            ...
            {['Tier 1 · Identity', 'Tier 2 · Address', 'Tier 3 · Institutional'].map((l, i) => (
              <div key={i} style={{ fontFamily: WFONT, fontSize: 11, fontWeight: 600, color: WBRAND.ink }}>
                ✓ {l}
              </div>
            ))}
          </div>
      ...
      <SectionCard title="Submitted documents">
        <div style={{ padding: '4px 22px 18px' }}>
          {[
            { l: 'Passport',                v: 'TR-U12345678 · uploaded Apr 30, 2024',  status: 'positive' },
            { l: 'Address proof',           v: 'Utility bill · uploaded May 1, 2024',    status: 'positive' },
            { l: 'Source of funds',         v: 'Bank statement · uploaded May 2, 2024', status: 'positive' },
            { l: 'Beneficial owner declaration', v: 'Notarised · uploaded May 2, 2024',  status: 'positive' },
            { l: 'Tax residency form (CRS)', v: 'TR · uploaded May 2, 2024',             status: 'positive' },
          ].map((d, i, arr) => (
            <div key={i} style={{
              display: 'flex', alignItems: 'center', gap: 14,
              ...
              <div style={{
                ...
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
                  <path d="M14 3H6a2 2 0 00-2 2v14a2 2 0 002 2h12a2 2 0 002-2V9l-6-6z" stroke={WBRAND.ink} strokeWidth="1.7"/>
                  <path d="M14 3v6h6" stroke={WBRAND.ink} strokeWidth="1.7"/>
                </svg>
              </div>
              <div style={{ flex: 1 }}>
        ...
        <div style={{ padding: '4px 22px 22px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12 }}>
            {[
              ...
              <div key={i} style={{
                padding: 16, border: `1px solid ${WBRAND.line}`, borderRadius: 12,
              }}>
...
// ─── Whitelisted destinations (crypto + bank + physical) ──────
function ProfDestinations() {
  const [tab, setTab] = useWProf('crypto');
  ...
  return (
    <SectionCard
      ...
      footer={
        <WPrimary>{WIcon.plus('#fff')} {
          tab === 'crypto' ? 'Add new address' :
          tab === 'bank'   ? 'Add bank account' :
    ...
    >
      {/* Tabs */}
      <div style={{
        padding: '0 22px', borderBottom: `1px solid ${WBRAND.line}`,
        display: 'flex', gap: 4,
      }}>
        {tabs.map(t => {
          const on = t.id === tab;
          return (
            <button key={t.id} onClick={() => setTab(t.id)} style={{
              padding: '12px 14px 14px', border: 'none', background: 'transparent',
              cursor: 'pointer', position: 'relative',
              fontFamily: WFONT, fontSize: 13, fontWeight: on ? 700 : 500,
              color: on ? WBRAND.ink : WBRAND.muted, letterSpacing: '-0.005em',
              display: 'flex', alignItems: 'center', gap: 8,
              marginBottom: -1,
            }}>
              <span>{t.label}</span>
              <span style={{
                ...
                background: on ? WBRAND.ink : WBRAND.surface,
                color: on ? '#fff' : WBRAND.muted,
        ...
        {tab === 'bank' && banks.map((b, i, arr) => (
          <div key={i} style={{
            ...
            <div style={{
              ...
              background: WBRAND.surface, display: 'grid', placeItems: 'center',
              fontFamily: WFONT, fontWeight: 800, fontSize: 11, color: WBRAND.ink, letterSpacing: '0.02em',
            }}>{b.ccy}</div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <div key={i} style={{
            ...
            <div style={{
              ...
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
                <path d="M12 21s-6-5.5-6-11a6 6 0 1 1 12 0c0 5.5-6 11-6 11z" stroke={WBRAND.ink} strokeWidth="1.7" strokeLinejoin="round"/>
                <circle cx="12" cy="10" r="2.2" stroke={WBRAND.ink} strokeWidth="1.7"/>
              </svg>
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
...
// ─── Banking ──────────────────────────────────────────────────
function ProfBanking() {
  return (
    <SectionCard title="Linked bank accounts" sub="Accounts used for fiat deposits and withdrawals." footer={<WPrimary>{WIcon.plus('#fff')} Add bank account</WPrimary>}>
      <div style={{ padding: '4px 22px 18px' }}>
        {[
          { bank: 'Emirates NBD',   ccy: 'AED', iban: 'AE07 0331 2345 6789 0001 198', def: true },
          { bank: 'JP Morgan Chase', ccy: 'USD', iban: '021000021 · 802135 2249',     def: false },
...
// ─── Whitelist ────────────────────────────────────────────────
function ProfWhitelist() {
  return (
    <SectionCard title="Whitelisted crypto addresses" sub="Withdrawals can only be sent to addresses you have pre-approved." footer={<WPrimary>{WIcon.plus('#fff')} Add new address</WPrimary>}>
      <div style={{ padding: '4px 22px 18px' }}>
        {[
          { l: 'Cold wallet · USDT', ch: 'Ethereum · ERC-20', a: '0xC4e1A3...8fB9hPq', added: 'Mar 20, 2026', verified: true },
          { l: 'Cold wallet · USDC', ch: 'Ethereum · ERC-20', a: '0xA1b2C3...4D5e6F7', added: 'Mar 22, 2026', verified: true },
...
// ─── Preferences ──────────────────────────────────────────────
function ProfPrefs() {
  const [lang, setLang] = useWProf('English · United Kingdom');
  const [ccy,  setCcy ] = useWProf('USDT — US dollar tether');
  const [tz,   setTz  ] = useWProf('Europe/Istanbul (UTC+03:00)');
        ...
        <div style={{ display: 'inline-flex', gap: 6 }}>
          {['Light', 'Dark', 'System'].map((m, i) => (
            <button key={m} style={{
              ...
              background: i === 0 ? WBRAND.ink : WBRAND.white,
              color: i === 0 ? '#fff' : WBRAND.ink,
              border: `1px solid ${i === 0 ? WBRAND.ink : WBRAND.line2}`,
              cursor: 'pointer', fontFamily: WFONT, fontSize: 12, fontWeight: 700,
            }}>{m}</button>
          ))}
        </div>
      </FormRow>
...
// ─── Select field (form-style dropdown) ───────────────────────
function SelectField({ value, options, groups, onChange }) {
  const [open, setOpen] = React.useState(false);

  // Normalize: prefer groups if provided
  ...
  return (
    <div style={{ position: 'relative' }}>
      <button onClick={() => setOpen(!open)} style={{
        ...
        background: WBRAND.white, border: `1px solid ${open ? WBRAND.ink : WBRAND.line2}`,
        cursor: 'pointer', textAlign: 'left',
      }}>
        <span style={{
          flex: 1, fontFamily: WFONT, fontSize: 13, color: WBRAND.ink, fontWeight: 500,
                ...
                {grp.options.map(o => {
                  ...
                  return (
                    <button key={o} onClick={() => { onChange(o); setOpen(false); }} style={{
                      ...
                      background: on ? WBRAND.surface : 'transparent',
                      cursor: 'pointer', textAlign: 'left',
                      fontFamily: WFONT, fontSize: 12, fontWeight: on ? 700 : 500, color: WBRAND.ink,
...
function Toggle({ on }) {
  return (
    <span style={{
      ...
      background: on ? WBRAND.red : WBRAND.line2,
...
function ProfAPI() {
  return (
    <SectionCard title="API access" sub="Programmatic access for institutional integrations." footer={<><WSecondary>Documentation</WSecondary><WPrimary>{WIcon.plus('#fff')} Generate key</WPrimary></>}>
      <div style={{ padding: '4px 22px 18px' }}>
        {[
          { name: 'Treasury reconciliation', ip: '188.40.x.x', scope: 'read-only', last: '4 hours ago' },
          { name: 'Risk dashboard',          ip: '52.84.x.x',  scope: 'read-only', last: '2 days ago' },
          { name: 'Production minting bot',  ip: '94.200.x.x', scope: 'mint, redeem', last: 'Just now' },
        ].map((k, i, arr) => (
          <div key={i} style={{
            display: 'flex', alignItems: 'center', gap: 14,
            padding: '14px 0', borderBottom: i === arr.length - 1 ? 'none' : `1px solid ${WBRAND.line}`,
            ...
            <div style={{
              width: 36, height: 36, borderRadius: 8,
              background: WBRAND.surface, display: 'grid', placeItems: 'center',
              fontFamily: WMONO, fontSize: 11, fontWeight: 700, color: WBRAND.ink,
            }}>API</div>
            <div style={{ flex: 1 }}>
              <div style={{ fontFamily: WFONT, fontSize: 13, fontWeight: 700, color: WBRAND.ink, letterSpacing: '-0.005em' }}>{k.name}</div>
              <div style={{ display: 'flex', gap: 8, marginTop: 3 }}>
                <WMonoNum size={11} color={WBRAND.muted}>{k.ip}</WMonoNum>
                <span style={{ width: 1, height: 10, background: WBRAND.line, alignSelf: 'center' }}/>
                <span style={{ fontFamily: WFONT, fontSize: 11, color: WBRAND.muted }}>{k.scope} · last used {k.last}</span>
              </div>
            </div>
            <WPill tone="positive">{WIcon.check(WBRAND.positive)} Active</WPill>
            <WSecondary size="sm">Rotate</WSecondary>
          </div>
        ))}
      </div>
    </SectionCard>
